...
class RuleBasedExtract:
    ...
    def _extract_phrase(self, phrases, tree_str, label, i, stop_words_set):
    ...
    def _extract_phrases(self, parser, local_sentences, stop_words_set):
        ...
        for i, sent in enumerate(local_sentences):
                ...
                phrases = self._extract_phrase(phrases, tree_str, "P", i, stop_words_set)
                # replace special tokens "-LRB" and "-RRB"
                for i, phrase in enumerate(phrases):
                    phrases[i][0] = phrase[0].replace("-LRB-", "(")
                    phrases[i][0] = phrase[0].replace("-RRB-", ")")
        
        phrases = list(map(lambda t : OriginalPhrase(t[0], t[1], t[2], t[3]), phrases))
